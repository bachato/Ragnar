...
- **Network Tools** — A built-in network engineer's toolbox in the web UI. **Diagnostics:** ping, traceroute, MTR, WHOIS, internet speed test, DNS Doctor (resolver / DNSSEC / poisoning checks), ARP Poisoning / MITM detection, MAC Watch (detection-only MAC-spoof + randomization + device tracking), DHCP Guardian (DHCP-snooping-style rogue-server + starvation detection), Path-MTU / black-hole probe, captive-portal check, LAN throughput via iperf3 (client + built-in server), Live Flow Telemetry (per-connection RTT / retransmits), and PTP timing (IEEE-1588 grandmaster) detection — plus an opt-in **Network Integrity Monitor** that runs the DNS-poison, ARP-spoof and rogue-DHCP checks on a schedule and Pushover-alerts when the verdict worsens. **Switch & L2:** switch discovery via LLDP / CDPv1v2 / EDP / FDP / SONMP with PoE detection, ARP host scanning, L2 link health, PCAP analyzer, and Locate Port (blinks a switch port so you can find the cable). **Interfaces:** per-interface link speed/duplex/auto-neg, DHCP-vs-static and VLAN detail, DNS/gateway network identity, per-interface public IP / ISP-ASN lookup (ISP/WAN detection) for multi-WAN setups, and a VPN egress check (catches a VPN running on the router). Missing CLI tools install with one click. Co-authored by [Solarflere](https://www.instagram.com/solarflere). See [Network Tools Guide](docs/nettools.md)
