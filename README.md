...
- **Network Tools** — A built-in network engineer's toolbox in the web UI. **Diagnostics:** ping, traceroute, MTR, WHOIS, internet speed test, DNS Doctor (resolver / DNSSEC / poisoning checks), ARP Poisoning / MITM detection, MAC Watch (detection-only MAC-spoof + randomization + device tracking), Path-MTU / black-hole probe, captive-portal check, LAN throughput via iperf3 (client + built-in server), Live Flow Telemetry (per-connection RTT / retransmits), and PTP timing (IEEE-1588 grandmaster) detection — plus an opt-in **Network Integrity Monitor** that runs the DNS-poison, ARP-spoof and rogue-DHCP checks on a schedule and Pushover-alerts when the verdict worsens. **Switch & L2/L3:** switch discovery via LLDP / CDPv1v2 / EDP / FDP / SONMP with PoE detection, ARP host scanning, DHCP Guardian (DHCP-snooping-style rogue-server + starvation detection) with an optional inline-bridge DHCP Snooping mode (trusted/untrusted ports + live binding table for a 2-NIC inline box), L2 link health, IGMP Watch (passive IGMP-snooping security scanner — multicast storm/flood, rogue-querier anomalies, group-enumeration reconnaissance, and unauthorized joins, detection-only with a small CLI + Scapy self-test), IPv6 First-Hop Watch (passive rogue-RA / rogue-DHCPv6 scanner — the overlooked SLAAC / mitm6 attack where a rogue Router Advertisement or DHCPv6 server silently becomes gateway/DNS on an "IPv4-only" network; flags rogue/high-pref/lifetime-0 RAs, injected RDNSS/prefixes, mitm6 DHCPv6 DNS-takeover, and RA floods, with RA-Guard advisories; detection-only, CLI + Scapy self-test), NTP Watch (passive rogue-NTP / clock-injection scanner — UDP/123 is the network's clock of record, and a rogue server serving the wrong time silently breaks TLS/Kerberos windows, MFA/TOTP, audit logs and lab-result timestamps; flags time-injection vs. the segment consensus/local clock, rogue + broadcast time sources, stratum-0 Kiss-o'-Death, Stratum-1/GPS stratum spoofing, mode-6/7 monlist recon, and dispersion/leap/refid anomalies, with NTS/monlist advisories; detection-only, CLI + Scapy self-test), ICMP Watch (passive ICMP-redirect / L3-injection scanner — the ICMP Redirect is the classic Layer-3 MITM where any host forges a gateway redirect to steer a victim's traffic through the attacker, no ARP poisoning needed; flags redirects steering to a non-gateway next-hop or from a non-gateway source, rogue IRDP router advertisements, ICMP floods/smurf, ICMP tunnelling (oversized echo exfil) and timestamp/address-mask/info recon, compared against the host's authoritative default gateway; detection-only, CLI + Scapy self-test), SNMP Watch (passive SNMP v1/v2c cleartext-exposure scanner — v1/v2c send the community string in cleartext, so anyone sniffing the segment harvests the device password; flags write-exposed SetRequests, cleartext community exposure with the actual strings seen and default/guessable ones called out, GetBulk amplification/reflection, and MIB-walk enumeration, with only SNMPv3 reading clean and a learned agent/community baseline that highlights new exposure; detection-only, CLI + Scapy self-test), TLS Watch (active certificate & TLS hygiene checker — internal admin UIs/NAS/hypervisors/IoT are full of unaudited certs, so it grades each target host:port for expired / not-yet-valid / self-signed / untrusted-chain / hostname-mismatch / weak-crypto (SHA-1/MD5, RSA<2048, weak cipher) / deprecated TLS 1.0-1.1 / expiring, fetching the cert even when validation fails to say why; an optional passive ClientHello-discovery pass finds TLS servers + SNI on the segment first, and a learned fingerprint baseline flags a cert that changed between scans (rotation or MITM); CLI + local-handshake self-test), OSPF Security Scanner (passive OSPF routing-security scanner — weak/no authentication, rogue routers + Router-ID conflicts, LSA injection / spoofed advertising router / MaxSeq-MaxAge / fight-back / Type-5 route injection, and flooding storms, with CVE/OSV advisories; detection-only, inspired by OSPFwatcher / FRR-MAD, CLI + Scapy self-test), BGP Path Watch (passive BGP routing-security scanner — prefix/origin & sub-prefix hijacks, rogue peers + session resets, bogon/reserved-ASN route leaks, BLACKHOLE communities, route-churn storms, and weak session security / no TCP-MD5, with CVE/OSV advisories; detection-only, L3-edge companion to OSPF, CLI + Scapy self-test), a **receive-only BGP collector + path-asymmetry** pairing (a from-scratch receive-only BGP speaker that learns a peer's RIB with per-prefix flap tracking but never advertises a route, plus a Paxson-min-pair one-way-delay probe that correlates a measured path-asymmetry shift back to control-plane routing truth — route-churn vs. data-plane), PCAP analyzer, and Locate Port (blinks a switch port so you can find the cable). **Interfaces:** per-interface link speed/duplex/auto-neg, DHCP-vs-static and VLAN detail, DNS/gateway network identity, per-interface public IP / ISP-ASN lookup (ISP/WAN detection) for multi-WAN setups, and a VPN egress check (catches a VPN running on the router). Missing CLI tools install with one click. Co-authored by [Solarflere](https://www.instagram.com/solarflere). See [Network Tools Guide](docs/nettools.md)
